...
class exportacion extends appDefault{
  ...
  public function insert(){
    ...
    $f=new complexForm('frm','Envio de Exportaci&oacute;n - '.$id,'','post');
    $f->addBody($f->genHidden('opc','guardar').$f->genHidden('id',$id).$f->genHidden('cp',$cp).
'<table>
	<tr>
		<td>Consignatario</td>
		<td><font color="blue"><u>'.$data->Fields('razon_extEmpresa').'</u></font></td>
	</tr>
	<tr>
		<td>T&eacute;rmino de Descarga</td>
		<td></td>
		<td>Fecha</td>
		<td>'.$f->genDateEdit('fechaFinDescarga_logDuim',dbDateToNormalDate($data->Fields('fechaFinDescarga_logDuim'))).'</td>
		<td>Hora</td>
		<td>'.$f->genTimeEdit('horaFinDescarga_logDuim',$data->Fields('horaFinDescarga_logDuim')).'</td>
	</tr>
	<tr>
		<td>Recepci&oacute;n</td>
		<td>'.$f->genInteger('recepcion',$recepcion).'</td>
		<td>Fecha</td>
		<td>'.$f->genDateEdit('fagente',$fagente).'</td>
		<td>Hora</td><td>'.$f->genTimeEdit('hagente',$hagente).'</td>
	</tr>
	<tr>
